...
<div class="table-card">
    ...
    <div class="table-scroll"><table>
        ...
        <tbody>
        @forelse ($data as $mhs)
            <tr><td class="muted">{{ $loop->iteration }}</td><td><span class="code-pill">{{ $mhs->nim }}</span></td><td><strong>{{ $mhs->nama }}</strong></td><td>{{ $mhs->no_telp }}</td><td>{{ $mhs->email }}</td><td>{{ $mhs->prodi }}</td><td><span class="status-pill">{{ $mhs->semester }}</span></td></tr>
        @empty
