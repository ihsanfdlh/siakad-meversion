...
<div class="table-card">
    ...
    <div class="table-scroll"><table>
        <thead><tr><th>No</th><th>Nama</th><th>Kepala Jurusan</th><th>Sekretaris Jurusan</th></tr></thead>
        <tbody>
        @forelse ($data as $jrsn)
            <tr><td class="muted">{{ $loop->iteration }}</td><td><strong>{{ $jrsn->nama }}</strong></td><td>{{ $jrsn->kajur->nama ?? 'Belum ada Kepala Jurusan' }}</td><td>{{ $jrsn->sekjur->nama ?? 'Belum ada Sekretaris Jurusan' }}</td></tr>
        @empty
