...
<div class="table-card">
    ...
    <div class="table-scroll"><table>
        <thead><tr><th>No</th><th>Nama</th><th>Jenjang</th><th>Kepala Program Studi</th><th>Jurusan</th></tr></thead>
        <tbody>
        @forelse ($prodis as $prodi)
            <tr><td class="muted">{{ $loop->iteration }}</td><td><strong>{{ $prodi->nama }}</strong></td><td>{{ $prodi->jenjang }}</td><td>{{ $prodi->kaprodi->nama ?? 'Belum ada Kaprodi' }}</td><td>{{ $prodi->jurusan->nama ?? 'Belum ada jurusan' }}</td></tr>
        @empty
